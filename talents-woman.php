...
<div class="content">
    ...
    <div class="filter-menu flex justify-center mt-52 text-white gap-6">
        ...
        <?php
            ...
            $femmes_category = get_term_by('slug', 'femmes', 'talent-category');

            if ($femmes_category) {
                // Récupérer dynamiquement toutes les sous-catégories de "Femmes"
                $subcategories = get_terms(array(
                    'taxonomy' => 'talent-category',
                    'hide_empty' => true,
                    'parent' => $femmes_category->term_id,
                ));

                // Inclure la catégorie principale "Femmes" dans les termes
                $terms = array_merge(array($femmes_category->slug), wp_list_pluck($subcategories, 'slug'));
            }
        ?>
